  ...
  if (current_settings->inside_data)
    ...
    if (current_settings->current_options->filter_action <= FILTER_DECISION_DO_FILTER)
      ...
      if ((current_settings->current_options->rejection != NULL) &&
        ...
        reset_rejection(current_settings, REJECTION_ACCESS_DENIED, &current_settings->current_options->rejection, &current_settings->current_options->rejection_buf, current_settings->current_options->reject_message_buf, MAX_BUF, NULL, NULL, 0);
    ...
    for (i = 0; current_settings->current_options->access_list_file[i] != NULL; i++)
      if ((line_num = search_tcprules_file(current_settings, command_text, MAX_BUF, current_settings->current_options->access_list_file[i], current_settings->server_ip, current_settings->server_name, current_settings->strlen_server_name)) >= 0)
        ...
        if ((command_text[0] == '\0') ||
          ...
          set_rejection(current_settings, REJECTION_ACCESS_DENIED, &current_settings->current_options->rejection, &current_settings->current_options->rejection_buf, current_settings->current_options->reject_message_buf, MAX_BUF, NULL, tmp_buf, current_settings->current_options->reject_reason_buf, MAX_BUF);
    ...
    if ((current_settings->server_ip != NULL) &&
      ...
      /* If the remote server's IP address is not a dotted quad, assume it is a name and look it up in DNS. */
        ...
        if (nihdns_a(current_settings, current_settings->server_ip, ip_ints, NULL, 0))
          ...
          snprintf(current_settings->tmp_server_ip, MAX_BUF, "%.*s%c%d.%d.%d.%d%n", strlen_environment_remote_ip[i], environment_remote_ip[i], ENVIRONMENT_DELIMITER, ip_ints[0], ip_ints[1], ip_ints[2], ip_ints[3], &current_settings->strlen_server_ip);
